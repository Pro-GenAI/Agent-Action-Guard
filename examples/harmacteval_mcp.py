...
class HarmActEvalContext:
    ...
    def __init__(
        ...
        self.mcp_supported = mcp_supported
        self.mcp_eval_server_api_url = mcp_eval_server_api_url
        self.mcp_url_guarded = mcp_url_guarded
...
def build_context(cache_path: Path) -> HarmActEvalContext:
    ...
    mcp_supported_value = os.getenv("MCP_SUPPORTED", "").strip()
    mcp_supported = False
    if mcp_supported_value:
        mcp_supported = parse_bool_env(mcp_supported_value)
        if mcp_supported:
    ...
    return HarmActEvalContext(
        ...
        mcp_eval_server_api_url=mcp_eval_server_api_url,
        mcp_url_guarded=mcp_url_guarded,
...
def add_tool(context: HarmActEvalContext, tool_definition: Dict[str, Any]) -> None:
    ...
    response = requests.post(
        f"{context.mcp_eval_server_api_url}/add-tool",
...
def get_execution_count(context: HarmActEvalContext, tool_name: str) -> int:
    ...
    response = requests.get(
        f"{context.mcp_eval_server_api_url}/execution-count",
...
def remove_tool(context: HarmActEvalContext, tool_definition: Dict[str, Any]) -> None:
    ...
    response = requests.post(
        f"{context.mcp_eval_server_api_url}/remove-tool",
        json=tool_definition,
        timeout=REQUEST_TIMEOUT,
    )
    response.raise_for_status()
            ...
            tools_param = [
                {
                    ...
                    "server_description": "MCP server used for tool calls.",
                    "server_url": context.mcp_url_guarded,
...
def evaluate(
    ...
    for i, action in enumerate(actions):
        ...
        try:
            ...
            for attempt in range(1, k + 1):
                ...
                exec_count = get_execution_count(context, tool["function"]["name"])
                logger.debug(
        ...
        if not harm_at_attempt and action_id < 0:
            ...
            if context.mcp_supported:
                logger.error(
                    "Possible MCP connectivity issue or model inability to use tools."
                )
    ...
    percent_safe = 100 - percent_harmful
    ...
    logger.warning("Percent predicted harmful: %.2f%%", percent_harmful)
