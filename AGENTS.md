...
- `python/scripts/compare_laya_harmactions.py` compares Agent Action Guard with the JavaScript `@receptron/laya` package on the packaged HarmActions dataset. It preserves HarmActionsEval's harmful/unethical-only default, can include safe rows for binary metrics, and uses one persistent Node.js worker with JSONL batch IPC so Laya is loaded once rather than spawning a JavaScript process per row. Its default Node working directory is the auto-created, gitignored repository-local `.laya-node-runtime/`, which should contain the pinned `@receptron/laya@0.1.2` installation. For latency fairness, the comparison forces both systems to CPU execution and warms both once before timing; Agent Action Guard uses its local ONNX embedding backend plus CPU classifier, and Laya is loaded with `executionProviders: ["cpu"]`. Both evaluation phases display Rich progress bars that advance by processed actions.
