...
The repository also ships a TypeScript-authored runtime package under `typescript/` that exposes `isActionHarmful()`, `ensureActionSafety()`, and `actionGuarded()` for Node.js tool screening.
...
For Node.js projects, use the npm package in `typescript/`.
...
- `python` — Python package for the classifier, dataset loading, and runtime helpers.
    ...
    - `python/experiments/` — research-only prototypes that are not part of the shipped package; includes the Mixture of Classifiers (MoC) router/expert prototype, HarmActions training, and same-device regular-vs-MoC HarmActionsEval comparison. Use `python/experiments/README.md` for CPU/GPU commands.
    - `python/examples/` — sample integrations and MCP server examples.
    - `python/tests/` — unit tests validating core behavior.
- `typescript/` — TypeScript-authored npm runtime package for Node.js action screening; `src/*.ts` compiles to publishable ESM and declarations in `dist/` via `npm run build`.
...
- PyPI publishing is handled by `.github/workflows/publish-pypi.yml` when a GitHub Release is created. The workflow builds the package, runs `twine check`, tests the built wheel in a clean venv, and publishes to PyPI using trusted publishing. The clean-wheel test installs repository-test-only `rich` alongside `pytest` because the Laya comparison regression test imports the manual benchmark script; `rich` remains outside the published package's runtime dependencies. Research-only MoC tests and Torch are not part of published-package verification. Bump `project.version` before publishing because PyPI does not allow replacing an existing release.
...
- Add all new user-facing usage/runtime features to both the Python and TypeScript packages so their public capabilities stay in sync. Training-only functionality belongs in Python only (for example under `python/training/`) and does not need a TypeScript counterpart.
...
- `python/scripts/compare_laya_harmactions.py` compares Agent Action Guard, the JavaScript `@receptron/laya` package, and an OpenAI LLM classifier on the packaged HarmActions dataset. It preserves HarmActionsEval's harmful/unethical-only default, can include safe rows for binary metrics, and uses one persistent Node.js worker with JSONL batch IPC so Laya is loaded once rather than spawning a JavaScript process per row. Its default Node working directory is the auto-created, gitignored repository-local `.laya-node-runtime/`, which should contain the pinned `@receptron/laya@0.1.2` installation. For latency fairness, the comparison forces both local systems to CPU execution and warms both once before timing; Agent Action Guard uses its local ONNX embedding backend plus CPU classifier, and Laya is loaded with `executionProviders: ["cpu"]`. The LLM model is read from the required `OPENAI_MODEL` environment variable, uses the OpenAI Responses API with reasoning effort `none` and strict JSON-schema Structured Outputs, evaluates only the action payload, and persistently caches results under `.cache/laya_comparison_llm_cache.json` using model/instructions/action-aware keys. All evaluation phases display Rich progress bars that advance by processed actions.
- Use `typescript/scripts/test-all-versions.js` for Node.js runtime compatibility testing. It must use nvm (sourcing `NVM_SH` or `$NVM_DIR/nvm.sh`) to install/select each requested runtime, run `npm ci` under that runtime, and then execute the JavaScript tests. The default matrix is Node.js 18, 20, 22, and 24; CLI version arguments override `NODE_TEST_VERSIONS`. Continue through failures by default and return non-zero after the matrix, with `CONTINUE_ON_FAILURE=0` available for fail-fast runs.
- Use `typescript/scripts/test-dependency-matrix.js` for JavaScript dependency compatibility testing. It must test `@huggingface/tokenizers` 0.1.x, ONNX Runtime Node 1.14+, and OpenAI JS 4.x-7.x across old, sampled, and recent releases; pack the local package and install it into isolated scenario projects so the selected versions are actually used; and return a non-zero exit status when any scenario fails. The base scenarios test the exact oldest/newest dependency profiles, and the matrix must query npm's current `latest` tag for every dependency and fail if that release falls outside the declared supported range; sampled scenarios vary one intermediate dependency release at a time against the oldest supported companion-dependency baseline to avoid redundant heavyweight installs. `make dependency-matrix-bounds` is the fast oldest/newest dependency check; `make dependency-matrix` runs the broader sampled matrix. Keep this separate from `test:all-versions`, which exercises supported Node.js runtime majors through nvm.
